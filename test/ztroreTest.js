...
describe("ztrore contracts", () => {
  const setup = async ({ maxSupply = 10000 }) => {
    const [owner] = await ethers.getSigners();
    const ztrore = await ethers.getContractFactory("ztrore");
    const deployed = await ztrore.deploy(maxSupply);
  ...
  describe("Minting", () => {
    ...
    it("Has a minting limit", async () => {
      ...
      await expect(deployed.mint()).to.be.revertedWith("No NFTs left :(");
    });
  });
