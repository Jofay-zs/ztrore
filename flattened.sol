...
contract ztroreDNA {
    ...
    function calculatingDNA(uint256 _tokenId, address _minter)
        public
        pure
        returns (uint256)
    {
        // We ask for the token and for the wallet address, so the nft created
        // doesn't matter if the token is the same, if someone else mines it,
        // it will be different
        uint256 combinedParams = _tokenId + uint160(_minter);
        bytes memory encodedParams = abi.encodePacked(combinedParams);
        // We hash this part to to make it difficult to predict the nft that
        // can be predicted, if I have enough time change this for Chainlink.
    ...
    function _getDNASection(uint256 _dna, uint8 _discard)
        internal
        pure
        returns (uint8)
    {
        return
            uint8(
                (_dna % (1 * 10**(_discard + DNA_SECTION_SIZE))) /
                    (1 * 10**_discard)
            );
    }
